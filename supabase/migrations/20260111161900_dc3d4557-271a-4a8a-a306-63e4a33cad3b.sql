-- Delete incorrectly created reminders for offer ARM/2026/0111/002
DELETE FROM offer_reminders WHERE offer_id = '706fe782-2b55-4438-a302-f273e5195dc3';

-- Update the create_offer_reminders function to properly filter by selectedOptionalItems for upsells
CREATE OR REPLACE FUNCTION create_offer_reminders(p_offer_id UUID, p_completed_at TIMESTAMPTZ)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF v_selected_state IS NOT NULL THEN
    ...
    -- Get selected optional items (item_id -> boolean) for upsells
    IF v_selected_state ? 'selectedOptionalItems' AND v_selected_state->'selectedOptionalItems' IS NOT NULL THEN
      FOR v_item_id IN SELECT jsonb_object_keys(v_selected_state->'selectedOptionalItems')
      LOOP
  ...
  FOR v_product IN 
    SELECT DISTINCT ON (pl.id)
      ...
      ooi.custom_name,
      oo.id as option_id,
      oo.is_upsell as is_upsell_option,
  ...
  LOOP
    ...
    -- For upsell options: check if this specific item was selected in selectedOptionalItems
    IF v_product.is_upsell_option = true AND array_length(v_selected_optional_item_ids, 1) > 0 THEN
      IF NOT (v_product.item_id = ANY(v_selected_optional_item_ids)) THEN
        CONTINUE; -- Skip upsell items not explicitly selected by customer
      END IF;
    END IF;
