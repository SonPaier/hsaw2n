-- Create a SECURITY DEFINER function to create a customer change request (public flow)
CREATE OR REPLACE FUNCTION public.request_reservation_change_by_code(
  _original_confirmation_code text,
  _new_reservation_date text,
  _new_start_time text,
  _new_service_id uuid,
  _new_station_id uuid DEFAULT NULL
)
...
BEGIN
  ...
  IF (_orig.reservation_date::date + _orig.start_time::time) < (now() + make_interval(hours => _cutoff_hours)) THEN
  ...
  _new_end_time := to_char((_new_start_time::time + make_interval(mins => _duration_minutes)), 'HH24:MI');
  ...
  SELECT
    ...
    COALESCE(s.name, 'Usługa') || ' - ' || _new_reservation_date || ' o ' || substring(_new_start_time from 1 for 5),
    'reservation',
    _new_id
  FROM public.services s
  WHERE s.id = _new_service_id;
...
-- Grant execute permission for public booking flow
GRANT EXECUTE ON FUNCTION public.request_reservation_change_by_code(text, text, text, uuid, uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.request_reservation_change_by_code(text, text, text, uuid, uuid) TO authenticated;
