...
const CustomersView = ({
  ...
  const fetchCustomers = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('customers').select('*').eq('instance_id', instanceId).order('name');
    if (!error && data) {
      setCustomers(data);
    }
    setLoading(false);
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(c => c.name.toLowerCase().includes(query) || c.phone.includes(query) || c.email && c.email.toLowerCase().includes(query));
    }
  ...
  if (loading) {
    return <div className="glass-card p-8 text-center text-muted-foreground">
        Ładowanie klientów...
      </div>;
  }
  return <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Szukaj po imieniu, telefonie lub email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
        </div>
      ...
      <div className="glass-card overflow-hidden">
        {paginatedCustomers.length === 0 ? <div className="p-8 text-center text-muted-foreground">
            {searchQuery ? 'Nie znaleziono klientów' : 'Brak klientów'}
          </div> : <div className="divide-y divide-border/50">
            {paginatedCustomers.map(customer => <div key={customer.id} onClick={() => setSelectedCustomer(customer)} className="p-4 flex items-center justify-between gap-4 transition-colors cursor-pointer bg-primary-foreground">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Button variant="ghost" size="icon" className="h-9 w-9 text-success hover:text-success hover:bg-success/10" onClick={e => handleCall(customer.phone, e)}>
                    <Phone className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-9 w-9 text-primary hover:text-primary hover:bg-primary/10" onClick={e => handleSms(customer, e)}>
      ...
      <CustomerDetailsDialog customer={selectedCustomer} instanceId={instanceId} open={!!selectedCustomer} onClose={() => setSelectedCustomer(null)} />

      {/* Send SMS Dialog (web only) */}
      <SendSmsDialog phone={smsCustomer?.phone || ''} customerName={smsCustomer?.name || ''} instanceId={instanceId} open={!!smsCustomer} onClose={() => setSmsCustomer(null)} />
    </div>;
};
